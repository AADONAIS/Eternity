...
public class ModifyItemUI extends JFrame {
	...
	public ModifyItemUI(final int index,String name,String nums,String money,String supplierName,String accountInfo,String time) {
		this.setTitle("修改商品");
		...
		updateButton.setBounds(90, 450, 75, 30);
		...
		updateButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
                     ...
                     if(Util.isAllDigit(accountField.getText())){
                    	 JOptionPane.showMessageDialog(new JFrame(), "数量输入不合法");
                     }
                     if(ispaybox.getSelectedItem().equals(ispaybox.getItemAt(0))){
                    	 JOptionPane.showMessageDialog(new JFrame(), "未选择是否付款");
                     ...
                     try {
						accountService.modify(index,accountBean);
		...
		closeButton.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				int result=JOptionPane.showConfirmDialog(new JFrame(),"是否关闭?","关闭提示ʾ",JOptionPane.YES_NO_OPTION);
				if(result==0) {
